function [stim, All_envs, ERBspace, Tones_f] = Stim_Bind_ABABA_chin(Corr_inds, fs, f_start, f_end, Tones_num, ERB_spacing)
...
% This version of the binding stimulus will return one stim in ABAB format
    ...
    [Tones_f, ERBspace] = Get_Tones_Chin(Tones_num, ERB_spacing, f_start, f_end); %returns tone frequencies 
    T = 0:1/fs:5*T_a-1/fs;
%    T = T(1:end-2);
    ...
    for k = 1:length(Tones_f)
        ...
        env_A3 = create_envelope(bw,lpf,T_a,fs);
        ...
        env = [env_A1; env_B1; env_A2; env_B2;env_A3];
