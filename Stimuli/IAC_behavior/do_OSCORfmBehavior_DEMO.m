...
L=70; %dB SPL
ntrials = 5;
FMs = [ones(1,ntrials)*5,ones(1,ntrials)*10,ones(1,ntrials)*20,ones(1,ntrials)*40];%,ones(1,ntrials)*80, ones(1,ntrials)*160];
%FMs = [ones(1,ntrials) * 20];
...
for i =1:ntrials*nconds
    ...
    fprintf(1, 'Response =%d, answer =%d, Correct = %d \n', resp, correctList(i),resp==correctList(i));
