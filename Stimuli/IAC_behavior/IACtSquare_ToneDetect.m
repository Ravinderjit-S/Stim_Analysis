...
%Tone_on_inds = Window_on_ind(end)-length(tone)+1:Window_on_ind(end);%put tone at end of square period
...
stim_nbnTone = nbn1;
if length(Tone_on_inds) - length(tone) == 1 % deal with some cases with tone_dur
    Tone_on_inds = Tone_on_inds(1:end-1);
end
