...
for k = 1:length(demo_phi)
    ...
    f2 = fratio*f1; 
    stim = FM_phi(f1,f2,fs,stim_dur,fm,demo_phi(k),diotic,ref);
    order = [1,2,3,4];
    ...
    if diotic
        fname = [folder_loc 'DEMO_FM_diotic' num2str(fm) '_phi_' num2str(demo_phi(k)) '_trial_' num2str(k) '.wav'];
    else
        fname = [folder_loc 'DEMO_FM_' num2str(fm) '_phi_' num2str(demo_phi(k)) '_trial_' num2str(k) '.wav'];
    end
    audiowrite(fname, stimulus_all',fs);
end

if diotic
    save([folder_loc 'DEMOprac_StimData_diotic' num2str(fm) '.mat'],'correct_k','fm','phis');
    save(['DEMOprac_StimData_diotic' num2str(fm) '.mat'],'correct_k','fm','phis');
else
...
for i = 1:length(phis)
    ...
    stimulus_all = scaleSound(stimulus_all);
    if diotic
        fname = [folder_loc 'DEMO_FM_diotic' num2str(fm) '_trial_' num2str(i) '.wav'];
    else
        fname = [folder_loc 'DEMO_FM_' num2str(fm) '_trial_' num2str(i) '.wav'];
    end
    audiowrite(fname, stimulus_all',fs);
end
if diotic
...
if diotic
    fname = [folder_loc 'DEMO_diotic_volstim.wav'];
else
