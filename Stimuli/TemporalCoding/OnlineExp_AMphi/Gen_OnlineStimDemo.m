...
for k = 1:length(demo_phi)
    ...
    f2 = fratio*f1; 
    stim = SAM_phi(f1,f2,fs,stim_dur,fm,demo_phi(k),diotic,ref);
    order = [1,2,3,4];
    ...
    if diotic
        fname = [folder_loc 'DEMO_AM_diotic' num2str(fm) '_phi_' num2str(demo_phi(k)) '_trial_' num2str(k) '.wav'];
    else
        fname = [folder_loc 'DEMO_AM_diotic' num2str(fm) '_phi_' num2str(demo_phi(k)) '_trial_' num2str(k) '.wav'];
    end
...
for i = 1:length(phis)
    ...
    stimulus_all = scaleSound(stimulus_all);
    if diotic
        fname = [folder_loc 'DEMO_AM_diotic' num2str(fm) '_trial_' num2str(i) '.wav'];
    else
