...
while demo1
    ...
    f2 = fratio*f1; 
    stim = FM_phi(f1,f2,fs,stim_dur,FMs_test(1),dichotics(end-1),dichotic_test); %first stim
...
while demo2
    f1 = randi(frange(2)/fratio - frange(1)) + frange(1); 
    f2 = fratio*f1; 
    stim = FM_phi(f1,f2,fs,stim_dur,FMs_test(3),dichotics(end-1),dichotic_test); %first stim
    info = sprintf('Answer is 3');
    info2 = sprintf('Press any button to play stim');
    Screen('DrawText',PS.window,info,textlocH,textlocV,PS.white);
    Screen('DrawText',PS.window,info2,textlocH,textlocV+100,PS.white);
...
f1 = randi(frange(2)/fratio - frange(1)) + frange(1); 
f2 = fratio*f1; 
stim = FM_phi(f1,f2,fs,stim_dur,FMs(1),dichotics(1),dichotic_test); %first stim
...
for i =1:ntrials*nconds
    ...
    for j = 1:3
        ...
        if j == 3 && i~= ntrials*nconds
            f1 = randi(frange(2)/fratio - frange(1)) + frange(1); 
            f2 = fratio*f1; 
            stim = FM_phi(f1,f2,fs,stim_dur,FMs(i+1),dichotics(i+1),dichotic_test); %first stim
            StimGenTime = toc();
        else
            StimGenTime = toc();
        end
