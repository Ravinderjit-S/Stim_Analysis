...
dur_og = dur;
extend = 999; %a value of 1 leads to doubling the signal
dur = dur*(1+extend); %to deal with edge effects making signal longer and then gonna chop off edges
fstep = 1/dur; 
ind_flow = round(flow/fstep);
ind_fhigh = round(fhigh/fstep);
...
fullspec2(ind_flow:ind_fhigh) = fullspec(ind_flow:ind_fhigh).*(exp(1j*p)); %This line adds the phases calculated in p to the phase of the noise
