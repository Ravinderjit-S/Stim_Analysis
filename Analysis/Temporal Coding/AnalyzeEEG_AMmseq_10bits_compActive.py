...
pickle_loc = data_loc + 'Pickles_compActive/'
...
#%% Get tot trials for active data
A_Tot_trials_act = []

dataActive_loc = '/media/ravinderjit/Data_Drive/Data/EEGdata/TemporalCoding/AMmseq_active/'
pickle_active_loc = dataActive_loc + 'Pickles/'

for sub in range(len(Subjects)):
    subject = Subjects[sub]
    with open(os.path.join(pickle_active_loc,subject +'_AMmseq10bits_Active.pickle'),'rb') as file:
...
for subject in Subjects:
    ...
    [210, 215, 195, 203, 214, 204, 177, 233]
    ...
    epdat = epdat[:,:A_Tot_trials_act[Subjects.index(subject)],:]
    print('Total Trials Left: ' + str(epdat.shape[1]))
    ...
    #%% Save Data
    with open(os.path.join(pickle_loc,subject+'_AMmseq10bits_compActive.pickle'),'wb') as file:
