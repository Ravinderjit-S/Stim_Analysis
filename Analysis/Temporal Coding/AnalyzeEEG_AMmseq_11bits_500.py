...
mseq_loc = '/media/ravinderjit/Data_Drive/Data/EEGdata/TemporalCoding/mseqEEG_500_bits11_4096.mat'
Mseq_dat = sio.loadmat(mseq_loc)
mseq = Mseq_dat['mseqEEG_4096'].astype(float)

data_loc = '/media/ravinderjit/Data_Drive/Data/EEGdata/TemporalCoding/AMmseq_11bits_500/'
...
for subject in Subjects:
    ...
    epochs.average().plot(titles='11 bit AMmseq')
    ...
    #%% Save Data
    with open(os.path.join(pickle_loc,subject+'_AMmseq11bits_500.pickle'),'wb') as file:
        pickle.dump([t, Tot_trials, Ht, Htnf, info_obj, ch_picks],file)
    
    with open(os.path.join(pickle_loc,subject+'_AMmseq11bits_epochs_500.pickle'),'wb') as file:
