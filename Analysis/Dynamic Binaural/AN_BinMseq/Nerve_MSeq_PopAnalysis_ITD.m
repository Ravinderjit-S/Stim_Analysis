...
%     for getPics = 1:numel(data_MOVN)
%         NOSCOR_pics(getPics) = data_MOVN{getPics}.General.picture_number;
%     end
    Already_analyzed = []; %if there are repeat stimuli ... they will be skipped here
    [SamePics, SameStim, SameUnit] = FindSamePics(data_MOVN);
    for i = 1:numel(data_MOVN)
        ...
        unit_i = MOVN_i.General.track + MOVN_i.General.unit/100;
...
%         R_spks = NOSCOR_i.MSO.RightSpks{1};
        [spontRate, drivenRate, spontSTD, drivenSTD] = SpontaneousRate(MOVN_i.MSO.LeftSpks{1},stim_dur,stim_per);    
        
        for k =1:numel(MOVN_i.MSO.H_imp)
            H_NFt = [];
            H_NFf = [];
            
            H_dummy = MOVN_i.MSO.H_NF{k};
...
%                 MSO_spks = size(MOVN_i.MSO.simulated{k},1);
%                 MSO_lines = MOVN_i.MSO.simulated{k}(end,1);
%                 for together = 1:numel(NOSCOR_Inds)
%                     MSO_spks = MSO_spks + size(data_MOVN{NOSCOR_Inds(together)}.MSO.simulated{k},1);
%                     MSO_lines = MSO_lines + data_MOVN{NOSCOR_Inds(together)}.MSO.simulated{k}(end,1);
%                     H_imp = H_imp+data_MOVN{NOSCOR_Inds(together)}.MSO.H_imp{k};
%                     H_dummy2 = data_MOVN{NOSCOR_Inds(together)}.MSO.H_NF{k};
...
%             end
            ...
            Roll_off_6dB = f_H(find(pow2db(H_impf) <= max(pow2db(H_impf))-6,1,'first')); %10 dB drop off in power point 
            ...
            subplot(3,1,1),plot((0:length(H_impt)-1)/fs .*1000, H_impt,'linewidth',2),title(['H(t) ' fig_Tit]),xlabel('Time (msec)'), ylabel('Firing Rate/ ITD')
            hold on
...
%             figure, semilogx(f_H,Z_Hf),title('Z_Hf')
            ...
            All_CF_RT = [All_CF_RT MOVN_i.tc.CFrealtime];
            ...
            All_Thresh_interp = [All_Thresh_interp MOVN_i.tc.ThreshInterp];
...
%Temp_Mask = ones(1,50); Temp_Mask(37:38) = 0; Temp_Mask(45:46)=0; %only here cause you haven't implemented combining 
...
Mask2 = Mask & All_spontRate < 40 & All_atten ==40;
Mask3 = Mask & All_spontRate >= 40 & All_atten ==40;

figure, plot(All_CF_interp(Mask2), All_spontRate(Mask2),'rx'), xlabel('CF'), ylabel('SpontRate')
hold on, plot(All_CF_interp(Mask3), All_spontRate(Mask3),'bx'), legend('Low Spont', 'High Spont')
figure, plot(All_CF_interp(Mask), All_Roll_off_Freq(Mask),'x'), xlabel('CF') , ylabel('Roll off Frequency (Hz)')

figure, plot(All_CF_interp(Mask2), All_Roll_off_Freq(Mask2),'rx'), xlabel('CF') , ylabel('Roll off Frequency (Hz)')
hold on, plot(All_CF_interp(Mask3), All_Roll_off_Freq(Mask3),'bx'), legend('Low Spont', 'High Spont')

figure, plot(All_CF_interp(Mask2), All_Roll_off_Freq10dB(Mask2),'rx'), xlabel('CF'), ylabel('Roll Off -10 dB')
hold on, plot(All_CF_interp(Mask3), All_Roll_off_Freq10dB(Mask3),'bx'), legend('Low Spont', 'High Spont')

figure, plot(All_CF_interp(Mask),All_Thresh_interp(Mask),'x'), xlabel('CF'), ylabel('Threshold')
...
figure, hold on, xlabel('CF'), ylabel('Roll off -10dB') 
...
figure
Mask2_40 = Mask_40;
%Mask3_temp = boolean([1,1,1,1,0,1,0,1,0,0]);
...
hold off
xlabel('CF (Hz)'), ylabel('Roll off frequency (Hz)'), ylim([100 400]), xlim([min(All_CF_interp2)-50 max(All_CF_interp2)+50])
yticks([100:100:400])
xticks([500:500:2500])
set(gca,'fontsize',25)
fig = gcf;
...
% figure
% Mask = Used==1;
% subplot(2,1,1), semilogx(All_CF_interp(Mask),All_Roll_off_Freq(Mask),'x'), xlabel('CF (Hz)'),ylabel('Roll off Frequency (Hz)'), title('Used')
% subplot(2,1,2), semilogx(All_CF_interp(Mask),abs(All_GroupDelay(Mask)),'x'),xlabel('CF(Hz)'), ylabel('Group Delay (sec)')
...
% Mask = Mseq1;
% Mask2 = Mseq2;
% subplot(2,1,1), semilogx(All_CF_interp(Mask),All_Roll_off_Freq(Mask),'rx'), xlabel('CF (Hz)'),ylabel('Roll off Frequency (Hz)'), title('Used ITD'), xlim([100 max(All_CF_interp(Mask))])
% hold on, semilogx(All_CF_interp(Mask2),All_Roll_off_Freq(Mask2),'bx'), hold off, legend('MseqDur: 1','MseqDur:2')
% subplot(2,1,2), semilogx(All_CF_interp(Mask),abs(All_GroupDelay(Mask)),'rx'),xlabel('CF(Hz)'), ylabel('Group Delay (sec)'), xlim([100 max(All_CF_interp(Mask))])
% hold on, semilogx(All_CF_interp(Mask2),abs(All_GroupDelay(Mask2)),'bx'), hold off
% 
% 
% 
