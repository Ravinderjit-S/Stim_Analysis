...
%             end
            ...
            ReallyUsed =  MSO_lines >100 & Roll_off>0 & Roll_off > Roll_off_10dB & (Mseq_StepDur == 1 | Mseq_StepDur ==2) & attenuation==40;
            
            if ReallyUsed
                saveas(gcf,[fig_loc '/ReallyUsed/' Analyze_dates{TheDate} '_Fig' num2str(i) '.' num2str(k) '.png'])
            end
            
            if MSO_lines <100
                Used = [Used 0];
                saveas(gcf,[fig_loc '/Not_Used/' Analyze_dates{TheDate} '_Fig' num2str(i) '.' num2str(k) '.png'])
            else
                Used = [Used 1];
