...
figure(), hold on
plot(f,pow2db(AcrossSubjectHf_IAC),'k','linewidth',3), xlim([0 30]), xlabel('Frequency (Hz)'), ylabel('Power (dB/Hz)')
Index_log = 2:length(f); %fill doesn't work on logscale if you include x data point of 0
plotsem = fill([f(Index_log);flipud(f(Index_log))],[AcrossSubjectHf_IAC_SEMdown(Index_log);flipud(AcrossSubjectHf_IAC_SEMup(Index_log))],'k','linestyle','none');
...
% Hf_IAC_6dBIndex = find(max(AcrossSubjectHf_IAC) - 6;

figure(), hold on
plot(f,AcrossSubjectHf_IAC,'k','linewidth',5), xlim([0 20]), xlabel('Frequency (Hz)'), ylabel('Power (dB/Hz)')
Index_log = 2:length(f); %fill doesn't work on logscale if you include x data point of 0
...
plot(f,NFmean_IAC,'r','linewidth',5)
